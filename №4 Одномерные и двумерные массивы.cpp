...
void print_min_circle(const T_points_set points_set) {
    ...
    if (equal_to_for_real(R_min, R_MIN_EMPTY)) {
        std::cout <<rus("Нет окружностей.")
                  << std::endl;
    } else {
        std::cout <<rus("Окружность с наименьшим радиусом ")
                  << R_min
                  <<rus(" имеет центр в точке ")
...
int main() {
    ...
    int n;
    do {
        std::cout <<rus("Введите количество точек >= 1: ");
        std::cin >> n;
    } while (n < 1);

    T_points_set points_set;
    std::cout <<rus("Введите координаты ")
              << n
              <<rus(" различных точек:")
              << std::endl;
